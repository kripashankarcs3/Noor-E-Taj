...
export default function BridalPlanner({ onBack, onNavigate, darkMode }) {
              ...
              {OTHER_MATCHES.map(m => (
                <div key={m.id} className="other-match-card">
                  ...
                  <div className="other-match-body">
                    ...
                    <button className="other-match-btn" onClick={() => { const el = document.createElement('div'); el.className = 'noor-toast'; el.textContent = `📖 Viewing "${m.title}" details...`; el.style.cssText = 'position:fixed;bottom:24px;left:50%;transform:translateX(-50%);padding:10px 24px;border-radius:var(--radius-sm);font-family:var(--font-btn);font-size:0.82rem;z-index:2000;box-shadow:0 8px 24px rgba(0,0,0,0.2)'; document.body.appendChild(el); setTimeout(() => el.remove(), 2500); }}>View Details</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="planner-form-area">
          ...
          <div className="results-header-row">
            ...
            <div className="results-action-btns">
              <button className="results-download-btn" onClick={() => { const planText = `Noor AI — Bridal Plan\n\nWedding Style: ${styleLabel} Traditional\nType: ${typeLabel}\nVenue: ${selectedVenue}\nMonth: ${selectedMonth}\nBudget: ${selectedBudget}\nFunctions: ${functions}\nColors: ${selectedColors.join(', ')}\nRequirements: ${requirements || 'None'}\n\nGenerated by Noor AI ✨`; const blob = new Blob([planText], {type: 'text/plain'}); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'Noor_AI_Bridal_Plan.txt'; a.click(); URL.revokeObjectURL(url); const el = document.createElement('div'); el.className = 'noor-toast'; el.textContent = '📄 Plan downloaded!'; el.style.cssText = 'position:fixed;bottom:24px;left:50%;transform:translateX(-50%);padding:10px 24px;border-radius:var(--radius-sm);font-family:var(--font-btn);font-size:0.82rem;z-index:2000;box-shadow:0 8px 24px rgba(0,0,0,0.2)'; document.body.appendChild(el); setTimeout(() => el.remove(), 2500); }}>⬇ Download Plan</button>
              <button className="results-save-btn" onClick={() => { const plan = { style: `${styleLabel} Traditional`, type: typeLabel, venue: selectedVenue, month: selectedMonth, budget: selectedBudget, functions, colors: selectedColors, requirements, savedAt: new Date().toISOString() }; localStorage.setItem('noor_saved_plan', JSON.stringify(plan)); const el = document.createElement('div'); el.className = 'noor-toast'; el.textContent = '✅ Plan saved to your account!'; el.style.cssText = 'position:fixed;bottom:24px;left:50%;transform:translateX(-50%);padding:10px 24px;border-radius:var(--radius-sm);font-family:var(--font-btn);font-size:0.82rem;z-index:2000;box-shadow:0 8px 24px rgba(0,0,0,0.2)'; document.body.appendChild(el); setTimeout(() => el.remove(), 2500); }}>🔖 Save Plan</button>
            </div>
          </div>
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: 'var(--radius-lg)', padding: '28px', width: 480, maxWidth: '90vw', maxHeight: '80vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div style={{ background: 'var(--sidebar-bg)', borderRadius: 'var(--radius-sm)', padding: '14px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: '0.9rem', fontWeight: 700, color: 'var(--text-white)', fontFamily: 'var(--font-btn)' }}>Estimated Range</span>
  ...
  return (
    <div className="planner-page animate-fade-in">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '14px' }}>
                  {ARTISTS.slice(0, 3).map(a => (
                    <div key={a.id} style={{ background: 'var(--card-bg-elevated)', border: '1px solid var(--card-border)', borderRadius: 'var(--radius-md)', padding: '16px', textAlign: 'center' }}>
                      <div style={{ width: 64, height: 64, borderRadius: '50%', overflow: 'hidden', margin: '0 auto 10px', background: 'var(--card-bg-elevated)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.5rem' }}>👩‍🎨</div>
          ...
          <div style={{ paddingTop: 12, borderTop: '1px solid rgba(0,0,0,0.06)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
              {['/priya_profile.png', '/digital_twin_portrait.png', '/priya_profile.png'].map((src, i) => (
                <img key={i} src={src} alt="Expert" style={{ width: 28, height: 28, borderRadius: '50%', objectFit: 'cover', border: '2px solid var(--card-bg)', marginLeft: i > 0 ? -8 : 0 }} />
              ))}
      ...
      {/* ── TOAST ── */}
      {toast && (
        <div style={{ position: 'fixed', bottom: 30, left: '50%', transform: 'translateX(-50%)', background: 'var(--sidebar-bg)', color: 'var(--text-white)', padding: '12px 24px', borderRadius: 'var(--radius-md)', fontFamily: 'var(--font-btn)', fontWeight: 600, fontSize: '0.85rem', boxShadow: '0 8px 24px rgba(0,0,0,0.2)', zIndex: 1001 }}>
          {toast}
        </div>
      )}
      ...
      {showCostEstimate && (
        <div onClick={() => setShowCostEstimate(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--card-bg)', borderRadius: 'var(--radius-lg)', padding: '28px', width: 480, maxWidth: '90vw', maxHeight: '80vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.3)', border: '1px solid var(--card-border)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: '1.2rem', fontWeight: 700, color: 'var(--sidebar-bg)', margin: 0 }}>💰 Cost Estimate</h3>
              <button onClick={() => setShowCostEstimate(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', fontSize: '1.1rem' }}>✕</button>
            </div>
